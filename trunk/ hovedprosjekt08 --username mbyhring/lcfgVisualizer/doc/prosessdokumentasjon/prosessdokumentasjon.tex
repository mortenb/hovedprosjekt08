...
\subsection{Prosjektets start} %vet ikke om vi trenger dette..
Prosjektet begynte den... bla bla bla.. 
Satte opp hjemmesiden og prosjektdagbok.
...
Fase en av projektet gikk i stor grad ut på å få en oversikt over strukturen på disse filene og hvilke data vi hadde tilgjengelig.
Oppdragsgiver hadde gitt oss en viss pekepinn på hvilke data som ikke var interessante. 
En del av dette ble fjernet, blant annet feilsøkingsinformasjon.
Dette halverte filstørrelsen, og gjorde det lettere å lese filene manuelt.
...
%\\
%\\
En oppsummering av oppbygningen av XML-filene:
\begin{itemize}
...
Etter å ha lest om forskjellige teknologier og vurdert alternativene, bestemte vi oss for å bruke Perl, VRML og mySQL. 
...
\\
\\
Vi har valgt å benytte mySQL som databasemotor.
Som programmerings-IDE har vi valgt å bruke eclipse, med tilleggsmodulene EPIC (Eclipse Perl Integration) og subclipse for versjonskontroll (Subversion).
Dette fordi det er et godt utviklingsmiljø til Perl, med integrert støtte for subversion.
\\
\\
...
Lineær Algebra - Matte 200 vektorer..
\\
\\
Systemutvikling  --skrive noe av dette
...
Dette har hjulpet oss å forstå mange av konfigurasjonsparametrene som forekommer i profilfilene.
Dermed har det blitt lettere å velge ut komponenter til visualisering. 
...
Gruppa hadde på forhånd ingen erfaring med visualisering eller 3d-programmering.
Så vi måtte bruke mye tid på å lære VRML og 3d-tankegang, samt få en bedre forståelse av visualiseringsteknikker og utnytte dette i vårt prosjekt.
...
\subsubsection{visualisering i praksis} %Bør flyttes.. 
...
I VRML kan fargene skrives som en tredimensjonal vektor der rød, grønn og blå komponentene
 har verdier mellom 0 og 1.
 
Utfordringer:
Menneskeøyet er ikke spesielt følsomt for farger, 
slik at vi bør tilstrebe at 
fargene som brukes er nokså forskjellige.
Dette blir naturligvis vanskeligere dersom vi 
har svært mange forskjellige kriterier.

 En løsning: Filtrere ut informasjon -- ta bort noen grupper når man studerer visualiseringen, for å ikke ta feil.
 
 
 Vi bør også sette en max-grense for antall farger -- 
 Over et visst punkt er det ikke lengere praktisk mulig å se
 forskjell på fargene, og da er det ikke hensiktsmessig 
 å bruke farger på kriteriet.
...
\\\\\noindent  
\subsubsection{XML-parser} %TODO: Putte dette et skikkelig sted
...
Vi løste midlertidig problemet med minneforbruket ved å kalle en doc->dispose() metode for hver fil vi hadde lest inn, fordi garbage collector ikke selv gjorde dette. Men fortsatt var vi ikke fornøyd med hastigheten.
...
Denne nye tabellen skal det legges data i, og senere skal den overskrive den gamle
 ...
 Løpe i gjennom filer ( foreach <>)
     ...
     Finne  "last\_modified"  og legge den i en variabel
     ...
     Sjekke \$changed
     Hvis "true"
     Oppdatere tabell  last\_modified med last\_modified 
