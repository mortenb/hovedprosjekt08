...
Issue: DataImporter. 
If you try to import a dataset of XML files which is older than the currently newest, the importer will 
When importing a dataset of XML files which is older than the recently imported, there is an issue with redundant data. Since the importer checks the values to be added against the newest values in the database, the set will not be added to the database unless there are different values.
