...
sub vectorColors() 
{
	...
	foreach 
	( my $r = 0 ; $r <= 1 ; $r += .25 ) {
		foreach ( my $vector = .25 ; $vector <= sqrt(3) ; $vector += .25 ) {
			my $maxr = 1;
			my $maxg = 1;
			if ( $vector < 1 ) 
			{
			...
			if ( $maxr**2 - $r**2 >= 0 ) {
				if ( $vector**2 - $r**2 < 1 ) 
				{
					$maxg = sqrt( $vector**2 - $r**2 );
				}

				foreach ( my $g = 0 ; $g <= $maxg ; $g += .25 ) 
				{
					if ( $vector**2 - $r**2 - $g**2 < 1 ) 
					{
						my $b = sqrt( $vector**2 - ( $r**2 + $g**2 ) );
	...
	";
#
#		]translation ".(int($int%8)*3)." ".(-int($int/8)*3)." 0
#		
#}\n";
		$int++;
