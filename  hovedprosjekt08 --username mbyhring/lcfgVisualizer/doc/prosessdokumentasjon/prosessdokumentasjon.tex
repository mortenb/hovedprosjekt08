...
\begin{itemize}
\item Simen Hagen - for veiledning gjennom hele prosjektperioden, og for prosjektfunn
\end{itemize}
...
LCFG er utviklet av blant annet Paul Anderson ved University of Edinburgh.
Det er et system for å administrere konfigurasjon av et stort antall UNIX-maskiner. Konfigurasjonsinnstillingene distribueres ved hjelp av XML-filer som hentes fra en webserver. Hver maskin har sin XML-fil med en unik konfigurasjon for den maskinen.
...
\subsection{Eksempel på bruk av programmet}
Systemadministrator ønsker å se fordeling av operativsystemer i nettverket sitt, og velger dette i brukergrensesnittet. Systemet henter verdier fra databasen, og genererer en VRML-fil som viser datamaskiner representert som 3D-objekter gruppert på operativsystem. Denne blir da presentert i brukergrensesnittet.

\subsection{Analyse av virkninger}
Sluttproduktet vil gjøre det mulig for en systemadministrator å få en oversikt over forholdsvis store datamengder, og få presentert sammenhenger som er vanskelig å lese ut manuelt.
...
\subsubsection{Valg av teknologier}
...
Vi løste midlertidig problemet med minneforbruket ved å kalle en \verb " doc->dispose() " metode for hver fil vi hadde lest inn, fordi Perls Garbage Collector ikke selv gjorde dette. Men fortsatt var vi ikke fornøyd med hastigheten.
