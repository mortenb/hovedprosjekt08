...
\newpage
\section{Innledning}
Dette dokument forklarer og beskriver produktet laget av gruppe 08-01. Det beskriver teknologier brukt og oppbygning av systemet. Dokumentet er laget med hensyns på veileder og sensor. Det kreves noe forkunnskap om informasjonsteknologi for å forstå helheten av dette dokumentet. Teksten er uthevet når det skrives om spesifikke deler av programmet, for å gjøre det lettere å skille mellom kode og tekst. Produktdokumentasjon er optimalisert for papirutskrift.

\section{Beskrivelse av programmet}
VSCD sitt bruksområde er uthentig av konfigurasjonsdata og visualisering av konfigurasjons(u)likhet.  Dataene hentes fra en ekstern LCFG-server (Local ConFiGuration system). LCFG er et system for automatisk installasjon og administrasjon av konfigurasjonen for et stort antall maskiner. Filene distribuert av LCFG blir tolket av VSCD, og konfigurasjonsdata som egner seg til visualisering blir lagret i intern database. Disse dataene blir brukt av VSCD sin visualiseringsmaskin til å produsere en tredimensjonal fremstilling av datamaterialet.

\section{Verktøy}
Programmet benytter seg av flere forskjellige teknologier for å oppnå sluttresultatet. Dette avsnittet gir en kort beskriver teknologiene som er benyttet i forbindelse med systemet. 
...
\subsection{MySQL}
My Structured Query Language er et system for administrasjon av realsjonsdatabaser. MySQL tilbyr et grensesnitt mellom en bruker og en database, hvor brukeren gis mulighet til å aksessere, endre og prosessere data i databaseen.
...
\subsection{Apache}
Apache er en web server som kan konfigureres til å kjøre eksterne programmer gjennom CGI og MySQL.



%I vårt program har også ukjente verdier i databasen en betydning for oss.
\section{Oppbygning av programmet}
Programmet består av to seksjoner. Den ene har som oppgave å tolke konfigurasjonsfilene og importere data til systemet. Den andre seksjonen tolker de importerte dataene og genererer visualiseringene. Programmet er i tillegg delt opp i en hierarkisk lagstruktur, der hvert lag har sitt ansvars- og bruksområde; dataaksess, visualisering og presentasjon. 

\begin{description}

\item[Importsdelen] har ansvaret for å sortere ut ønsket informasjon fra XML-filer, og legge dette inn i en intern database. Dataimportereren henter parameter fra en sentral konfigurasjonsfil, og setter opp ønsket datastruktur ut i fra disse. XML-filene blir tolket, verdier blir ekstrahert og sendt videre til DAL (dataaksesslag) for å bli lagret.
...
Den lokale konfigurasjonsfilen inneholder de komponentene som skal hentes ut fra XML-filene og  legges til i databasen, og namespace som tilhører dem. 

Importereren henter først ut all informasjon fra konfigurasjonsfilen, og redegjør for hva som skal bli tabeller og kolonnenavn i databasen. Den sjekker mot databasen om tabellene eksisterer fra før, og gjør eventuelle endringer hvis databasen ikke samsvarer med gjeldende konfigurasjon. Den bruker modulen \verb"Perl::LibXML" til å tolke profilfilene. I hver fil er det en obligatorisk komponent som kalles \verb"<last_modified>", som angir den datoen siste endring er blitt gjort fra LCFG-server. Denne datoen blir hentet opp av scriptet, som går videre til neste fil hvis maskin og dato allerede er lagt inn i database. Selv om det er ny dato i forhold til den som finnes i databasen, betyr ikke det at dataene som skal importeres er endret.  Derfor kontrollerer scriptet om de nye verdiene allerede ligger der, og legger dem bare  til hvis de faktisk er endret.
...
Hver gang et DAL-objekt blir opprettet, vil den opprette en hash som tilsvarer verdier skrevet i konfigurasjonsfilen. DAL bruker verdiene i denne hashen som parameter, og de blir primært brukt til å koble seg opp til databasen.

Metodene for import og ekstrahering av verdier er generiske. Ved henting av informasjon fra databasen, trenger metodene tabellnavn som parameter. Det som i hovedsak skiller disse metodene fra hverandre er oppbygningen til SQL-spørringene og hvilken struktur de dataene som returneres fra metodene har.
...
\end{description}
Metoden bruker først hjelpemetoder innad i klassen, for å hente parameter fra sentral konfigurasjonsfil. Der ligger blant annet tilkoblingsinformasjon til databasen. Returnerer så en referanse til seg selv som objekt.
...
\item[Returverdi:] Tabell med strenger som representerer VSCD-tabellene i databasen
\end{description}
Søker etter tabeller i databasen som oppfyller våre spesifikke krav til tabellstruktur (\verb"machinename" og \verb"last_modified" som primærnøkkel) og returnerer en tabell over disse. Ved import av en ekstern database er denne nyttig, siden det da er milig at tabellene i den nye databasen ikke samsvarer med konfigurasjonsfilen. Visualisereren vil dermed fortsatt kunne bruke den nye databasen.
...
\begin{description}
\item[]
\item[Antall parameter:] 3 \linebreak (Streng tabellnavn, Streng feltnavn, Streng kriterie)

\item[Returverdi:] Hash (Maskinnavn \verb"=>" Kriterie)
\end{description}
Henter alle noder som oppfyller valgt kriterium i databasen. 
