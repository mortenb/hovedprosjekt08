...
\item[$\triangleright$] inv / manager -$>$ Ansvarlig administrator
\item[$\triangleright$] profile / group -$>$ Gruppen som maskinen hører til. Kan si noe om funksjonen til maskinen
\item[$\triangleright$] inv / location -$>$ Fysisk lokasjon
\item[$\triangleright$] xinetd / enableservices -$>$ Tjenester som benyttes av xinet.d. Ikke mange profiler som har denne.
\item[$\triangleright$] apache -$>$ Betyr at maskinen kjører en webservertjeneste. Kun 17 av 43 har apacheconfig
\item[$\triangleright$] mysql -$>$ Betyr at maskinen har mysql daemon kjørende
\item[$\triangleright$] dhcpd -$>$ Betyr at maskinen kjører en dhcpdaemon
\item[$\triangleright$] postgresql -$>$ Betyr at maskinen har en postgresql daemon kjørende
\item[$\triangleright$] rsync -$>$ Betyr at maskinen har en rsync tjeneste oppe
\item[$\triangleright$] subversion/cvs -$>$ subversion tjeneste kjører
\item[$\triangleright$] samba -$>$ sambaserver kjører
\item[$\triangleright$] inv / model -$>$ forteller om maskinmodellen (eks: Dell OptiPlex 9700)
